...
async function importAllRelationships() {
  ...
  try {
    ...
    const files = (await fs.readdir(TARGET_DIR)).filter(f => f.endsWith('_relationship.json'));
...
async function processFile(file) {
  ...
  try {
    ...
    logger.info(`Successfully processed relationship from file ${file}`);
...
importAllRelationships().catch(err => logger.error(`Fatal error: ${err.message}`));
