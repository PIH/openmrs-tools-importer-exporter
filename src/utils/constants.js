...
const PERSON_REP = `(uuid,gender,birthdate,birthdateEstimated,dead,deathDate,causeOfDeath,` +
  `names:(uuid,preferred,prefix,givenName,familyName,familyName2,familyNamePrefix,familyNameSuffix,middleName,degree),` +
  `addresses:(preferred,address1,address2,address3,address4,address5,address6,address7,address8,address9,address10,address11,address12,address13,address14,address15,cityVillage,stateProvince,postalCode,countyDistrict,country,latitude,longitude,startDate,endDate),` +
  `attributes:(uuid,value,attributeType:(uuid)))`

const OBS_BASE_REP = `(uuid,concept:(uuid),person:(uuid),obsDatetime,location:(uuid),encounter:(uuid),comment,accessionNumber,formNamespaceAndPath,status,valueModifier,valueCodedName:(uuid),value:(uuid),GROUP_MEMBERS)`
...
export default {
  ...
  PATIENT_CUSTOM_REP: `v=custom:(uuid,display,allergyStatus,identifiers:(uuid,identifier,identifierType:(uuid),preferred),person:${PERSON_REP})`,
  VISIT_CUSTOM_REP: `v=custom:(uuid,patient:(uuid),attributes:(uuid,value,attributeType:(uuid)),startDatetime,stopDatetime,indication:(uuid),location:(uuid),visitType:(uuid))`,
