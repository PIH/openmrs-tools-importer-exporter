...
export default {
  SOURCE: {
    URLS: {
      ...
      MEDICATION_DISPENSE: `${config.OPENMRS_SOURCE_CONTEXT_PATH}/ws/rest/v1/medicationdispense`,
  ...
  TARGET: {
    URLS: {
      ...
      MEDICATION_DISPENSE: `${config.OPENMRS_TARGET_CONTEXT_PATH}/ws/rest/v1/medicationdispense`,
  ...
  MEDICATION_DISPENSE_CUSTOM_REP: `v=custom:(uuid,patient:(uuid),encounter:(uuid),concept:(uuid),drug:(uuid),location:(uuid),dispenser:(uuid),drugOrder:(uuid),status:(uuid),statusReason:(uuid),quantity,quantityUnits:(uuid),dose,doseUnits:(uuid),route:(uuid),frequency:(uuid),asNeeded,dosingInstructions,datePrepared,dateHandedOver,wasSubstituted,substitutionType:(uuid),substitutionReason:(uuid),dateCreated,creator:(uuid),dateChanged,changedBy:(uuid))`,
